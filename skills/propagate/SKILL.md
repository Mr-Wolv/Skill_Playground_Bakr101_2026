...
If the CLI tools are not available, derive test obligations manually from the spec using the test-generation taxonomy (see the Allium `references/test-generation` docs if the Allium CLI is installed).
